...
export class CommentService {
  ...
  getAll() {
    return this.http.get('http://127.0.0.1:8000/comments/', this.jwt()).map((response: Response) => response.json());
  }

  // -> http GET http://127.0.0.1:8000/users/1/
  getById(id: number) {
    return this.http.get('http://127.0.0.1:8000/comments/' + id + '/', this.jwt()).map((response: Response) => response.json());
  }

  // -> http POST http://127.0.0.1:8000/users/ username=... email=... password=...
  create(post: Post) {
    return this.http.post('http://127.0.0.1:8000/comments/', post, this.jwt()).map((response: Response) => response.json());
  }
